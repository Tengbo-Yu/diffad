...
class CosformerAttention(nn.Module):
    ...
    def forward(
        ...
        if self.causal:
            ...
            kv_ = torch.matmul(k_.unsqueeze(3), v.unsqueeze(2))
            # (N * h, L, 2 * d, d) -> (N * h, L, 2 * d, d)
            kv_cum = torch.cumsum(kv_, dim=1)
            # (N * h, L, 2 * d) (N * h, L, 2 * d, d) -> (N * h, L, d)
            qkv = torch.matmul(q_.unsqueeze(2), kv_cum).squeeze(2)
            # (N * h, L, 2 * d) -> (N * h, L, 2 * d)
            k_cum = torch.cumsum(k_, dim=1)
            # (N * h, L, 2 * d) (N * h, L, 2 * d) -> (N * h, L)
            denom = torch.clamp_min(torch.matmul(q_.unsqueeze(2), k_cum.unsqueeze(3)).squeeze(2).squeeze(2), eps)
            ...
            kv_ = torch.matmul(k_.transpose(1, 2), v)
            # (N * h, L, 2 * d) (N * h, 2 * d) -> (N * h, L)
            z_ = 1 / torch.clamp_min(torch.matmul(q_, torch.sum(k_, axis=1).unsqueeze(2)).squeeze(2), eps)
            # (N * h, L, 2 * d) (N * h, d, 2 * d) (N * h, L) -> (N * h, L, d)
            attn_output = torch.mul(torch.matmul(q_, kv_), z_.unsqueeze(2))
            # (N * h, L, d) -> (L, N * h, d) -> (L, N, E)
            attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, -1)
        # L, N, E
        if self.has_outproj:
