...
from diffusion import create_ddpm_diffusion


class DDPMDiffad(BaseDiffad):
    def __init__(self, config, num_inference_steps):
        super().__init__(config)
        self.diffusion = create_ddpm_diffusion(timestep_respacing="", learn_sigma=False)
    ...
    def forward(self, latent_z, plan_traj, plan_traj_mask, image, lidar2img, lidar2global, model_kwargs):
        ...
        t = torch.randint(0, self.diffusion.num_timesteps, (bs,), device=latent_z.device)  # bs
    ...
    def sample_single_step(self, cur_image, cur_lidar2img, cur_lidar2global, prev_h, prev_c, prev_x_start,
        ...
        samples = self.diffusion.ddim_sample_loop(
            self.model['ldm'], cur_z.shape, cur_z, clip_denoised=False, model_kwargs=model_kwargs,
            progress=True, device=cur_z.device, eta=1,
        )
        # extract trajectory from bev latent
        pred_trajs = self.model['postprocess'](samples) if 'postprocess' in self.model else torch.zeros(cur_z.shape[0],
                                                                                                        1, 12, 2)
