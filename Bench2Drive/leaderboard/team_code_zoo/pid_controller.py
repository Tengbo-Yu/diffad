...
class PIDController(object):
    ...
    def control_pid(self, waypoints, speed, target, delta_t=0.5):
        ...
        for i in range(num_pairs):
            ...
            desired_speed += np.linalg.norm(
                    waypoints[i+1] - waypoints[i]) / num_pairs / delta_t
        ...
        # Override if there's a significant deviation between predicted and target,
        # and if target is in far distance.
        angle_diff = np.abs(angle) - np.abs(angle_target)
        deviation_threshold = 45 / 180 * np.pi  # 30 / 180 * np.pi
        ...
        if use_target_to_aim:
            ...
            # avoid of abrupt turn
            if hasattr(self, 'angle_final'):
                angle_final = 0.7 * angle_final + 0.3 * self.last_angle
        else:
            angle_final = angle
        self.last_angle = angle_final
